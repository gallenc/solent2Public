...
public class ExampleProjectRestImpl {
    ...
    public Response retrieveMatchingEntites(Thing thingTemplate) {

        try {
            if (thingTemplate == null) {
                throw new IllegalArgumentException("thingTemplate request parameter must be set");
            }
            ReplyMessage replyMessage = new ReplyMessage();

            ServiceFacade serviceFacade = WebObjectFactory.getServiceFactory().getServiceFacade();
            List<Thing> eList = serviceFacade.retrieveMatchingThings(thingTemplate);

            LOG.debug("/retrievematching thingTemplate: " + thingTemplate 
                    + " found " + eList.size() + "things");

            replyMessage.getThingList().getThings().addAll(eList);
    ...
    public Response retrieve(@QueryParam("id") Integer id) {
        try {
            ...
            ServiceFacade serviceFacade = WebObjectFactory.getServiceFactory().getServiceFacade();
            Thing thing = serviceFacade.retrieveThing(id);
            if (thing != null) {
                LOG.debug("/retrieve id=" + id + " found thing :" + thing);
                replyMessage.getThingList().getThings().add(thing);

                replyMessage.setCode(Response.Status.OK.getStatusCode());
                return Response.status(Response.Status.OK).entity(replyMessage).build();
            } else {
                LOG.debug("/retrieve id=" + id + " found no thing :");
                replyMessage.setDebugMessage("/retrieve id=" + id + " found no thing");
                replyMessage.setCode(Response.Status.OK.getStatusCode());
                return Response.status(Response.Status.OK).entity(replyMessage).build();
            }
